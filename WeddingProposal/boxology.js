...
function validateCurrentStep() {
  ...
  switch(currentStepId) {
    case 'step1':
      ...
      break;
    case 'step2':
      if (!answers.age && !answers.exactAge) {
    ...
    case 'step6':
      ...
      break;
    case 'step7':
      // Handle custom budget validation
      if (!answers.budget) {
        alert('Bitte wähle eine Option aus');
        isValid = false;
...
// Unified event listener for all quiz interactions
document.addEventListener('click', function(e) {
  ...
  // Handle next button clicks (unified for all next button classes)
  if (e.target.classList.contains('quiz-next') || e.target.classList.contains('next-step')) {
    e.preventDefault();
    
    if (validateCurrentStep()) {
      if (currentStep === totalSteps - 1) {
        // This is the contact form step - handle form submission
  ...
  // Handle back button clicks (unified for all prev button classes)
  if (e.target.classList.contains('quiz-prev') || e.target.classList.contains('prev-step')) {
    e.preventDefault();
    if (currentStep > 0) {
      showStep(currentStep - 1);
    }
...
function resetQuiz() {
  ...
  // Hide special inputs
  const specialInputs = ['other-person-input', 'other-relationship-input', 'custom-budget-input'];
...
function updateSelectedInterests() {
  if (!selectedTagsContainer || !selectedInterestsContainer) return;
